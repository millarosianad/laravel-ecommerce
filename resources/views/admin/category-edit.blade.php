    ...
    <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="max-w-3xl mx-auto">
            <form action="{{ route('admin.category.update', ['id'=>$category->id]) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 space-y-6">
                ...
                {{-- Blok Alert Error --}}
                {{-- @if ($errors->any())
                    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg text-sm space-y-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" placeholder="e.g. Samsung" class="w-full border px-4 py-2 rounded-lg outline-none focus:ring-1 focus:ring-primary" value="{{$category->name}}" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" id="slug" name="slug" placeholder="samsung" class="w-full border px-4 py-2 rounded-lg bg-gray-50 outline-none" value="{{$category->slug}}" readonly>
                        @error('slug')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <select name="parent_id" id="parent_id" class="w-full border px-4 py-2 rounded-lg outline-none focus:ring-1 focus:ring-primary">
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="status" name="status" value="1" class="w-4 h-4 text-primary rounded border-gray-300 focus:ring-primary" {{ $category->status ? 'checked' : '' }}>
